...
本仓库不是 .NET 项目，因此仅在本仓库优先使用 C# 扩展而不启动 C# Dev Kit，避免其显示“无解决方案”；
此设置不会影响其他 VS Code 文件夹中的 C# Dev Kit。
...
打开要运行的 `.cpp` 文件后，按 **F6**（C/C++ Compile Run）即可编译并在 VS Code 集成终端运行；
也可以按 **Ctrl+Alt+N**（Code Runner）。两种方式都会先保存当前文件，`F6` 的可执行文件放在
被 Git 忽略的 `build/` 目录，Code Runner 的临时可执行文件运行后会清理。
如果程序使用 `cin`，终端看起来暂时没有输出是因为它正在等输入；在集成终端输入测试数据并回车即可。

微软 C/C++ 扩展自带的 `Run C/C++ File` 使用其构建任务；默认构建配置会在源码旁边生成 `.exe`，
这是程序的编译产物。日常单文件练习推荐使用 **F6** 或 **Ctrl+Alt+N**。
**Ctrl+Shift+B 已专门绑定为 Git 提交并推送，不是编译快捷键。**
